 ...
 * tweets in the user's news feed. Your design should support the following methods:
 ...
 user's news feed. Each item in the news feed must be posted by users who t
 he user followed or by the user herself. Tweets must be ordered from most recent to least recent.
...
public class Solution355 {
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        if (fans.containsKey(userId)) {
            for (int f : fans.get(userId)) {
                if (tweets.containsKey(f)) {
                    tweets.get(f).forEach(feed::add);
                }
            }
        }
